/** Molde y pautas de Órdenes médicas (paridad con OrdenesMedicasDefaults.kt). */

export const ORDENES_SECTION = "Órdenes";

export type OrdenesModo = "emergencia" | "ambulatorio" | "ordenes";

export const ORDENES_MODO_LABELS: Record<OrdenesModo, string> = {
...
export const ORDENES_MOLDE_EJEMPLO = [
...
export function resolveOrdenesMolde(custom?: string): string {
  const t = custom?.trim();
  return t || ORDENES_MOLDE_EJEMPLO;
}

function modeGuidelines(modo: OrdenesModo): string {
  switch (modo) {
    case "emergencia":
      return [
...
/** Pautas fijas + molde personalizado para el prompt de IA. */
export function ordenesMedicasPromptBlock(
  moldePersonalizado?: string,
  modo: OrdenesModo = "ordenes",
): string {
  const molde = resolveOrdenesMolde(moldePersonalizado);
  return [
